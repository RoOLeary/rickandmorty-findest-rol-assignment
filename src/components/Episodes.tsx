...
const Episodes = () => {
  ...
  const [showAllCharacters, setShowAllCharacters] = useState<{
    [key: number]: boolean;
  }>({});
  ...
  const LoadingFallback = () => (
    <div>Awwww Jeez...th..th..this is gonna take a second.</div>
  );
  ...
  const fetchCharacterDetails = async (
    ...
    setCharacterDetails((prev) => ({
      ...prev,
      [episodeId]: characterDetailsArray,
    }));
  };
            ...
            data?.results?.map((episode: any) => (
              <div key={episode.id} className="episodeListItem">
                <div className="episodeListItemDetail">
                    ...
                    <Suspense fallback={<LoadingFallback />}>
                      {characterDetails[episode.id] ? (
                        characterDetails[episode.id]
                          .slice(
                            0,
                            showAllCharacters[episode.id]
                              ? characterDetails[episode.id].length
                              : 9,
                          )
                          .map((character, idx) => (
                            <li key={idx}>
                  ...
                  {characterDetails[episode.id] &&
                    characterDetails[episode.id].length > 9 && (
                      <button
                        onClick={() => handleToggleShowAll(episode.id)}
                        className="text-lg font-black text-[#08BAE3] hover:text-[#66BA4F]"
                      >
                        {showAllCharacters[episode.id]
                          ? "Show Fewer"
                          : "Show All Characters"}
